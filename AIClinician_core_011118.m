...
% GENERATES
        ...
        % MIMICzs = MIMIC ZSCORED m*47 array with columns in right order
...
disp('####  INITIALISATION  ####') % BY SAMPLING
...
colbin=find(ismember(reformat5t.Properties.VariableNames,colbin));colnorm=find(ismember(reformat5t.Properties.VariableNames,colnorm));collog=find(ismember(reformat5t.Properties.VariableNames,collog));

% find patients who died in ICU during data collection period
ii=reformat5t.bloc==1&reformat5t.died_within_48h_of_out_time==1& reformat5t.delay_end_of_record_and_discharge_or_death<24;
icustayidlist=reformat5t.icustayid;
ikeep=~ismember(icustayidlist,reformat5t.icustayid(ii));
reformat5=table2array(reformat5t);
reformat5=reformat5(ikeep,:);
icustayidlist=reformat5t.icustayid(ikeep);
...
MIMICraw=reformat5t(ikeep, [colbin colnorm collog]);
...
coltbin=find(ismember(valid50.Properties.VariableNames,coltbin));coltnorm=find(ismember(valid50.Properties.VariableNames,coltnorm));coltlog=find(ismember(valid50.Properties.VariableNames,coltlog));

valid50.sysbp(valid50.sysbp<5)=NaN;
valid50.diabp(valid50.diabp<5)=NaN;
valid50.meanbp(valid50.meanbp<5)=NaN;

Xtest=valid50(:, [coltbin coltnorm coltlog]);
...
% if size(valid50keep,1)~= size(eICUraw,1); disp('Check valide50keep and eICUraw'); disp('EXECUTION STOPPED'); return;end
...
% create new version of QLDATA3 (NO INTERMEDIATE REWARD in this version)
r=[100 -100];
r2=r.*(2*(1-Y90)-1); 
...
idxtest=fastknnsearch(Xtestmimic,C,1);    
  ...
  idxtest2=cell(size(eICUzs,1),1);
    ...
    tic
      parfor i=1:size(eICUzs,1)
        idxtest2(i)={fastknnsearch(eICUzs(i,~ii(i,:)),C(:,~ii(i,:)),1)};  %which ones are the k closest records in Xtrain? - only match on available data (ii columns)!
      end
    toc
...
ptid=valid50.patientunitstayid;
blocstest=valid50.bloc;
...
bestpol=r(max(r(:,24))==r(:,24),1);   % model maximising 95% LB of value of AI policy in MIMIC test set is selected 
...
[idx]=fastknnsearch(MIMICzs(train,:),C,1);  %N-D nearest point search: look for points closest to each centroid
[~,OptimalAction]=max(Qon,[],2);  %deterministic 
...
% show that the value of AI policy is always guaranteed to be better than doctors'
...
%% FIG 2B BOXPLOT OF POL VALUE OVER 500 MODELS -  MIMIC TEST SET ONLY

figure
clear h
boxplot(recqvi(:,[20 22 25 26]),{'Clinicians','AI','Zero drug','Random'});
...
% TD learning of physicians / bootstrapped. This version also records return and mortality, for the plot (nb: no parfor here)

disp('####   MODEL CALIBRATION - CLINICIANS POLICY EVALUATION WITH Q LEARNING   ####')
tic
[bootql,prog]=offpolicy_eval_tdlearning_with_morta( qldata3train, physpol, ptid,  idx, actionbloctrain, Y90, 0.99, 100 );
toc
...
%% FIG SA - FEATURE IMPORTANCE for VASOPRESSORS, with bootstrap
...
collog={'SpO2','BUN','Creatinine','SGOT','SGPT','Total_bili','INR','input_total_tev','input_4hourly_tev','output_total','output_4hourly'};
v=reformat5t(1,[colbin colnorm collog]).Properties.VariableNames; %get he right column names!
...
iol=find(ismember(reformat5t.Properties.VariableNames,{'input_4hourly'}));
vcl=find(ismember(reformat5t.Properties.VariableNames,{'max_dose_vaso'}));
